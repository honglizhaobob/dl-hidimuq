...
dt = 1/max(max(max(abs(u)))/dx,max(max(abs(v)))/dy);
nt = ceil((tend/dt) + 1);
dt = tend/(nt-1);
...
% I.C.
% Gaussian mean = [-1.00125, -0.411665], sd = 0.1
z= ((Xg + 1.00125).^2 + (Yg + 0.411665).^2)/(0.1^2);
p(3:end-2,3:end-2) = exp(-0.5*z(3:end-2,3:end-2))/(2*pi*.1^2);
...
% speeds defined at left and bottom cell edges
% cell edges
[Yge1,Xge1] = meshgrid(yge,xge);
% cell centers
[Yg1,Xg1] = meshgrid(yg,xg);

u = Xge1 - (Xge1.^3)/3 - Yg1 + H;
v = ep*(Xg1 + alpha - beta*Yge1);
...
% I.C.
% Gaussian mean = [-1.00125, -0.411665], sd = 0.1
z= ((Xg1 + 1.00125).^2 + (Yg1 + 0.411665).^2)/(0.1^2);
p1(3:end-2,3:end-2) = exp(-0.5*z(3:end-2,3:end-2))/(2*pi*.1^2);
...
%     size(xWp)
    ...
    p1(3:end-2,3:end-2) = p1(3:end-2,3:end-2) - dt*((Apdq + Amdq)/dx ...
        + (Bpdq + Bmdq)/dy + (Fp(:,2:end-1)-Fm(:,2:end-1))/dx ...
        + (Gp(2:end-1,:)-Gm(2:end-1,:))/dy);
    
end
p2 = interp2(Yg1,Xg1,p1,Yg,Xg);

err(kk) = max(max(abs(p2(3:end-2,3:end-2)-p(3:end-2,3:end-2))));
err2(kk) = sqrt(trapz(Yg(1,3:end-2),trapz(Xg(3:end-2,1),(p2(3:end-2,3:end-2)-p(3:end-2,3:end-2)).^2,1)));
h(kk) = 5/NN(kk);
end

close all;
figure(1)
loglog(h,h,'-b',h,h.^2,'-r',h,err,'-k')
figure(2)
loglog(h,h,'-b',h,h.^2,'-r',h,err2,'-k')
