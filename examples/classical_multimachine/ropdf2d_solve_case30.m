...
% set up pde domain
dx = 0.02; 
dy = 0.01;    % spatial step size
ng = 2;             % number of ghost cells

% left right boundaries
x_min = -5*dx; x_max = max(mc_energy1,[],'all')-std(mc_energy1,[],"all");
nx = ceil((x_max-x_min)/dx);

% top bottom boundaries
y_min = -5*dy; y_max = max(mc_energy2,[],'all');
...
% save benchmark PDF
fname = "./data/case30/joint_benchmark.mat";
if ~isfile(fname)
...
fname = "./data/case30/joint_observer.mat";
if ~isfile(fname)
    % benchmark for MC convergence study
    p_bench = load("./data/case30/joint_benchmark.mat").p_bench;
...
%% Compute L^1 error
tmp = load("./data/case30/joint_benchmark.mat");
p_bench = tmp.p_bench;
nx = tmp.nx; ny = tmp.ny; nt = tmp.nt;
dx = tmp.dx; dy = tmp.dy; dt = tmp.dt;
p_obs = load("./data/case30/joint_observer.mat").p_obs;
mcro = 2.^(9:13);
nmcro = length(mcro);
kde_error = zeros(nmcro,nt);
for i = 1:nmcro
    tmp = squeeze(p_obs(i,:,:,:));
    for nn = 1:nt
        err = trapz(dy,trapz(dx,abs(tmp(3:end-2,3:end-2,nn) ...
            -p_bench(3:end-2,3:end-2,nn))));
        disp(err);
        kde_error(i,nn) = err;
    end
end
...
fname = "./data/case30/joint_coeffs.mat";
if ~isfile(fname)
    % benchmark for MC convergence study
    p_bench = load("./data/case30/joint_benchmark.mat").p_bench;
    % number of reduced MC trials
...
sgtitle('Case 30',"FontSize",50,"FontName","Times","FontWeight","bold"); 
...
c.FontSize = 24;
% save figure
fname = "./fig/case30/joint/predicted2d.png";
...
save("./data/case30/joint_nudged.mat", "p", "lr");

%% Load solutions
load("./data/case30/joint_nudged.mat", "p", "lr");
...
% failure levels
failure_level1 = 1.3; failure_level2 = 0.65;
...
% product measure predictions
tail_pred_prod = zeros(nt,1);
tail_kde_prod = zeros(nt,1);
for nn = 1:nt
    disp(nn)
...
[~, idx] = max(tail_kde);
% load 1d ROPDFs
tmp1 = load("./data/case30/line1_ropdf.mat");
tmp2 = load("./data/case30/line2_ropdf.mat");
xpts = tmp1.xpts;
ypts = tmp2.xpts;
dx = xpts(2)-xpts(1);
dy = ypts(2)-ypts(1);
...
[~, idx] = max(tail_kde);
% load 1d ROPDFs
tmp1 = load("./data/case30/line1_kde.mat");
tmp2 = load("./data/case30/line2_kde.mat");
xpts = tmp1.xpts;
ypts = tmp2.xpts;
dx = xpts(2)-xpts(1);
dy = ypts(2)-ypts(1);
