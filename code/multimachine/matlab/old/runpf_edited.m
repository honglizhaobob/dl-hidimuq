function [dist, Pm, Ybus, vi, delta, success] = runpf_edited(casedata, mpopt, fname, solvedcase)
...
%
%   Outputs (all are required):
%       Ybus : complex admittance matrix 
%       delta : equilibrium voltage angles (radians)
%       vi : equilibrium voltage magnitudes
%       Pm : equilibrium total bus power injections
%       dist: used to compute branch distances
...
% FOR COMPUTING branch distances
dist = mpc.branch(:,[F_BUS, T_BUS, BR_X, BR_B]);

% output voltage magnitudes:
vi = mpc.bus(:,VM);

% output voltage angles in radians: 
delta = mpc.bus(:,VA)*pi/180;

% bus power injections
Pm = real(makeSbus(baseMVA, bus, gen, mpopt, V));
% Pm = real(V.* conj(Ybus*V));

% for case9: dmat = repmat(delta,1,9); 
%%%% what's below should be close to Pm:
% vi.*sum(repmat(vi',9,1).*(gg.*cos(dmat - dmat') + bb.*sin(dmat - dmat')),2);
